...
import {
  SELF_EVAL_PERIOD,
  downloadSelfEvalPdf,
  printSelfEvalDocument,
} from "@/lib/selfEvalDocument";
...
export default function ReflectionPage() {
    ...
    const check = async () => {
      ...
      try {
        ...
        if (typeof window !== "undefined") {
          try {
            if (draftRow != null) {
              localStorage.setItem("teacher_mate_goal_achievement_" + emailKey, String((draftRow.goal_achievement_text as string) ?? ""));
              localStorage.setItem("teacher_mate_reflection_text_" + emailKey, String((draftRow.reflection_text as string) ?? ""));
            } else {
              const goalToStore = localGoal || "";
              const reflectionToStore = localReflection || "";
        ...
        if (!draftRow && (localGoal || localReflection)) {
        ...
        // 작성일 연도가 비어 있으면 올해로 채움 (PDF에 '년'만 찍히는 경우 방지)
        if (!String(merged.dateYear ?? "").trim()) {
          merged.dateYear = String(new Date().getFullYear());
        }
  ...
  const handleSelfEvalPdf = () => {
    // 최신 입력값(연도 포함)이 캡처에 반영되도록 ref 사용
    void downloadSelfEvalPdf(selfEvalFormRef.current);
  };
  const handleSelfEvalPrint = () => {
    void printSelfEvalDocument(selfEvalFormRef.current);
  };
          ...
          <TabsContent value="evidence" className="mt-0 space-y-4">
            <Card className="rounded-2xl border-slate-200/80 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Button type="button" size="sm" variant="outline" className="rounded-full border-slate-300" onClick={handleSelfEvalPdf} title="PDF 파일로 바로 다운로드">
                    <FileDown className="mr-1.5 h-3.5 w-3.5" />
                    PDF로 저장
                  </Button>
                  <Button type="button" size="sm" variant="outline" className="rounded-full border-slate-300" onClick={handleSelfEvalPrint}>
                    <Printer className="mr-1.5 h-3.5 w-3.5" />
                    프린트
                  </Button>
                </div>
              </div>
              <p className="mt-1 text-xs text-slate-500">근무성적평정의 신뢰성과 타당성이 보장되도록 객관적 근거에 따라 종합적으로 평가하여야 한다. PDF·인쇄 시 다른 보고서와 같이 쪽번호와 여백이 적용됩니다.</p>
              <div className="mt-3 space-y-3">
                ...
                <div className="flex flex-wrap items-end justify-between gap-4 border-t border-slate-200 pt-4">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Input
                      value={selfEvalForm.dateYear}
                      onChange={(e) => setSelfEvalForm((p) => ({ ...p, dateYear: e.target.value }))}
                      placeholder="2026"
                      inputMode="numeric"
                      className={selfEvalFieldClass("dateYear", "w-16 rounded-lg text-sm")}
                      aria-label="년도"
                    />
                    <Label className="text-xs text-slate-600 shrink-0">년</Label>
                    <Input
                      value={selfEvalForm.dateMonth}
                      onChange={(e) => setSelfEvalForm((p) => ({ ...p, dateMonth: e.target.value }))}
                      placeholder="3"
                      inputMode="numeric"
                      className={selfEvalFieldClass("dateMonth", "w-12 rounded-lg text-sm")}
                      aria-label="월"
                    />
                    <Label className="text-xs text-slate-600 shrink-0">월</Label>
                    <Input
                      value={selfEvalForm.dateDay}
                      onChange={(e) => setSelfEvalForm((p) => ({ ...p, dateDay: e.target.value }))}
                      placeholder="15"
                      inputMode="numeric"
                      className={selfEvalFieldClass("dateDay", "w-12 rounded-lg text-sm")}
                      aria-label="일"
                    />
                    <Label className="text-xs text-slate-600 shrink-0">일</Label>
                  </div>
                </div>
