...
[출력] 위 진단과 계획서를 비교한 뒤, 5문장 이내로 **솔직한** 멘토링 코멘트를 작성할 것. 부족하면 부족하다고, 엉망이면 질책하고, 정말 잘 썼을 때만 칭찬할 것.`;
      ...
      const periodInstruction = spec.keys.includes("period")
        ? `\n[시기(period) 규칙 – 반드시 준수]\n- 연도는 **${currentYear}년**(올해)만 사용할 것. 과거 연도 사용 금지.\n${
            cardType === "book"
              ? `- 전문 서적/에듀테크는 **1학기**와 **2학기**를 적절히 나눌 것. 예: ${count}개면 앞쪽 절반은 "${currentYear}년 1학기 (3월~6월)", 뒤쪽 절반은 "${currentYear}년 2학기 (9월~12월)" 형식으로 배분.\n`
...
${cardType === "training" ? `[{"name":"AI 활용 수업 연수","period":"${currentYear}년 4월","duration":"8","remarks":"직무연수"}]` : ""}
${cardType === "expense" ? `[{"activity":"학부모 공개수업","period":"${currentYear}년 3월","method":"학부모 참관","remarks":""}]` : ""}
${cardType === "book" ? `[{"title":"회복적 생활교육","period":"${currentYear}년 1학기 (3월~6월)","method":"매주 1회 독서 및 핵심 정리"},{"title":"ChatGPT 교육 활용","period":"${currentYear}년 2학기 (9월~12월)","method":"수업 자료 제작에 활용"}]` : ""}
${cardType === "education" ? `[{"area":"조깅","period":"${currentYear}년 1학기","duration":"주 2회","remarks":""}]` : ""}
