...
각 문단은 명확한 주제를 다루되, 전문 컨설턴트가 결과를 해석하고 제안하는 톤으로 서술할 것.`;
      ...
      prompt = `[역할] 너는 교원 역량 개발을 지원하는 전문 컨설턴트이다. 사전사후 검사를 비교하여 분석해줄거야야
...
[내용] 아래 사전·사후 진단 결과를 비교하여, **3문단 내외**로 작성해 줘.
- 1문단: "분석 결과"로 시작하고, 전체 총점 변화(전·후)를 한 번 짚고, 가장 많이 **향상된 영역 2~3개**를 구체적인 점수 차이와 함께 설명한다. (예: "△△ 영역은 2.8점에서 3.7점으로 0.9점 향상되었습니다.")
...
- 3문단: 앞으로 1년 동안 "잘 유지해야 할 강점"과 "집중 보완이 필요한 약점"을 한 번씩 정리하고, 사전·사후 변화 흐름을 이어 가기 위한 간단한 실천 방향을 제시한다.
