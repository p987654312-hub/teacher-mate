...
export default function DiagnosisResultCharts({
  ...
  if (isPost && radarCompareData && barChartDataByDomain && barChartDataByDomain.length > 0) {
    // compact 모드(보고서 인쇄 전용)는 레이아웃을 세로 스택으로 단순화해서
    // 방사형 그래프 카드 안에서 시각적으로 정확히 가운데에 오도록 맞춘다.
    const radarHeight = compact ? 140 : 320; // px 기준
    if (compact) {
      ...
      return (
        <div className="flex flex-col gap-4">
          ...
          <Card className="rounded-2xl border-slate-200/80 bg-gradient-to-br from-slate-50/90 via-white to-violet-50/50 px-4 py-3 flex flex-col items-stretch">
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="w-full max-w-xl" style={{ height: radarHeight }}>
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart outerRadius="60%" data={radarCompareData}>
          ...
          {/* 아래: 막대그래프 카드를 2열로 배치 (보고서 인쇄용) */}
          <div className="grid grid-cols-2 gap-3 items-start">
            <div className="flex flex-col gap-3">{leftCol.map(renderBarCard)}</div>
            <div className="flex flex-col gap-3">{rightCol.map(renderBarCard)}</div>
          </div>
        </div>
      );
    }
