...
총점(100점 만점 환산): {{postTotal}}점`,
  ...
  result_report: {
    ...
    template: `[역할]
너는 마일리지 실천 기록을 항목별로 정리해 '결과 보고서'를 만드는 편집기다. 수치(목표·달성률)는 이미 시스템이 정확히 계산해 [확정 달성률 줄]로 제공한다. 너는 절대로 스스로 개수를 세거나 달성률(%)을 계산하거나 목표 숫자를 바꾸지 마라. 너의 임무는 단 두 가지다.
(1) 각 항목의 제목·목표·달성률 줄은 [확정 달성률 줄]에 적힌 6줄을 순서대로, 글자·숫자·기호 하나 바꾸지 말고 그대로 복사한다.
(2) 각 제목 줄 바로 아래에 그 항목의 마일리지 원본 기록을 복사해 불릿(- )으로 붙인다.

[확정 달성률 줄 — 토씨 하나 안 틀리게 그대로 복사]
{{achievementSummary}}

[불릿(원본 기록) 작성 규칙]
- {{mileageText}}에서 해당 항목의 \`[카테고리명]\`으로 시작하는 줄을 찾아, 맨 앞의 카테고리 태그(여는 대괄호 \`[\` + 카테고리명 + 닫는 대괄호 \`]\` + 바로 뒤 공백 1칸)를 **통째로 제거**하고 나머지 문장을 토씨 하나 틀리지 않게 복사한다.
  · 올바른 예: \`[직무연수] 26.03.20(금) 미세먼지 관련 보건연수 2시간 (26.03.26)\` → \`26.03.20(금) 미세먼지 관련 보건연수 2시간 (26.03.26)\`
  · 잘못된 예(금지): \`직무연수] 26.03.20(금) ...\` 처럼 카테고리명이나 닫는 대괄호 \`]\`가 일부라도 남으면 안 된다. 제목 줄에 이미 항목명이 있으므로 불릿 안에는 절대 항목명을 쓰지 마라.
- 어떤 항목이 [확정 달성률 줄]에서 어떤 카테고리인지는 \`**[카테고리명]**\`의 이름으로 매칭한다. 이름이 정확히 같은 마일리지 줄만 그 항목 아래에 넣는다.
- 해당 항목의 실제 기록 줄 수를 N이라 한다.
  · N이 1~3개: 있는 줄만 그대로 출력하고 종료한다. '외'라는 글자를 쓰지 마라.
  · N이 4개 이상: 1·2번째 줄은 그대로 출력하고, 3번째 줄 끝에 한 칸 띄우고 \` 외 [N-3]건\`을 붙인 뒤 종료한다. (4번째 줄부터는 절대 출력 금지)
- 기록이 하나도 없는 항목은 제목·목표·달성률 줄만 출력하고 불릿은 비운다.

[최종 출력 포맷]
(보고서 외 어떤 안내 문구도 출력하지 마라. [확정 달성률 줄]의 6줄 순서·양식을 그대로 지킨다.)

<확정 달성률 1번째 줄을 그대로>
  - [원본 복사 기록 1]
  - [원본 복사 기록 2]
  - [원본 복사 기록 3] (N이 4개 이상이면 끝에 ' 외 N건' 결합 후 종료)

<확정 달성률 2번째 줄을 그대로>
  - [내용]

…(이런 식으로 [확정 달성률 줄]의 6줄을 모두, 순서대로)
