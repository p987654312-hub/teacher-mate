...
총점(100점 만점 환산): {{postTotal}}점`,
  ...
  result_report: {
    ...
    template: `[역할]
너는 주어진 데이터를 기반으로 정량 목표 달성도를 정제하는 '독립형 데이터 프로세서'이다. 생성 능력, 요약 능력, 유연성을 모두 제외하고 오직 규칙에 따른 기계적 텍스트 분리 및 매칭만 수행하라.

[🚨 데이터 원천 격리 규칙 - 환각 절대 금지]
1. **[텍스트 출처는 오직 마일리지뿐]**: 하위 불릿( - )에 출력할 모든 문장은 100% {{mileageText}} 내에 존재하는 텍스트여야 한다. 
2. **[계획서 텍스트 사용 금지]**: {{planSummary}}에 적힌 세부 내용(예: "AI 기반 기초학력...", "심화 연수" 등)은 가상의 데이터로 취급하여 **단 한 글자도 출력 결과에 포함하지 마라.** {{planSummary}}에서는 오직 [항목명]과 [목표 숫자]만 읽고 즉시 뇌에서 지워라.

[출력 라인 및 '외 00' 제한 규칙]
- 각 항목당 하위 불릿( - )은 **최대 3줄을 절대 초과할 수 없다.**
- 문장 맨 앞의 \`[직무연수]\`, \`[수업 공개]\` 같은 대괄호 태그는 삭제하고 내용만 출력한다.
- **기록이 1~3개인 경우**: 실제 존재하는 개수만큼만 출력하고 끝낸다. (중복 출력 금지)
- **기록이 4개 이상인 경우**: 
  * 1번째 기록 출력
  * 2번째 기록 출력
  * 3번째 기록 끝에 한 칸 띄우고 \`외 [전체 개수 - 3]건\` (또는 \`외 [남은 시간 합산]시간\`)을 붙이고 **출력을 즉시 강제 종료**한다. 4번째 기록부터는 절대로 화면에 뿌리지 마라.

[정밀 수치 계산 규칙]
- 달성률은 **오직 {{mileageText}}에 적힌 기록들로만 계산**한다. (계획서에 적힌 시간은 절대 더하지 마라)
- 건수/횟수 항목: {{mileageText}}의 해당 항목 총 줄 수를 세어 계산.
- 시간 항목: {{mileageText}} 문장 내에 포함된 시간(예: 1시간, 40분 등)을 모두 합산하여 계산. (소수점 버림, 최대 100%)

[최종 출력 포맷]
(인사말, 안내 문구 없이 바로 아래 결과만 끝까지 출력할 것. 항목 간 1줄 공백)

**[항목명]** 목표 : [목표 수치] 이상 ( [달성률]% 완료)
  - [mileageText에서 가져온 실제 기록 1]
  - [mileageText에서 가져온 실제 기록 2] (기록이 2개 이상일 때만)
  - [mileageText에서 가져온 실제 기록 3] (기록이 3개일 때만 출력. 만약 4개 이상이면 끝에 '외 00건' 또는 '외 00시간'을 붙이고 끝낼 것)
